                ...
                <div class="member-card" data-member-id="${memberId}" data-source="${searchResult.source}" data-index="${index}">
                  <div class="member-card-header">
                    ...
                    <div class="member-card-actions">
                      ...
                      <button class="enroll-fingerprint-btn secondary-btn" data-member-id="${memberId}" data-index="${index}">Enroll Fingerprint</button>
                    </div>
                  </div>
                  <div class="member-card-details">
                    <p><strong>Email:</strong> ${email}</p>
                    <p><strong>Member ID:</strong> ${memberId || 'N/A'}</p>
                    ${member.confidenceLevel ? `<p class="confidence-level ${member.confidenceLevel.toLowerCase().replace(' ', '-')}"><strong>Match Confidence:</strong> ${member.confidenceLevel}</p>` : ''}
                    ${member.matchDetails && member.matchDetails.length > 0 ? `
                      <div class="match-details">
                        <p><strong>Match Details:</strong></p>
                        <ul>
                          ${member.matchDetails.map(detail => `<li>${detail}</li>`).join('')}
                        </ul>
                      </div>
                    ` : ''}
                  </div>
