...
async function processScan(scan) {
  ...
  try {
    if (!scan) {
      ...
      resultDiv.textContent = 'Fetching latest scan...';
      const latestScan = await window.scanidAPI.getLatestScan();
      
      if (!latestScan.success) {
        resultDiv.textContent = `Error: ${latestScan.error}`;
        return;
      }
      
      if (!latestScan.scan) {
        resultDiv.textContent = 'No scan data available. Please scan an ID first.';
        return;
      }
      
      scan = latestScan.scan;
    }
      ...
      <div class="scan-card">
        ...
        <div class="scan-body">
          <div class="scan-field">
            ...
            <span class="field-value">${scan.FirstName || ''} ${scan.MiddleName || ''} ${scan.LastName || ''}</span>
          </div>
          <div class="scan-field">
            <span class="field-label">DOB:</span>
            <span class="field-value">${scan.DateOfBirth || 'N/A'}</span>
          </div>
          <div class="scan-field">
            <span class="field-label">ID #:</span>
            <span class="field-value">${scan.DocumentNumber || scan.LicenseNumber || 'N/A'}</span>
          </div>
          <div class="scan-field">
            <span class="field-label">State:</span>
            <span class="field-value">${scan.State || 'N/A'}</span>
          </div>
        </div>
      </div>
    `;
    ...
    // Enhanced debugging for the search result
    console.log('Raw search result from Wix API:', searchResult);
    console.log('Search result success:', searchResult.success);
    console.log('Search result source:', searchResult.source);
    console.log('Has results property:', searchResult.hasOwnProperty('results'));
    console.log('Results length:', searchResult.results ? searchResult.results.length : 'N/A');
    console.log('Has contacts property:', searchResult.hasOwnProperty('contacts'));
    console.log('Contacts length:', searchResult.contacts ? searchResult.contacts.length : 'N/A');
    ...
    // Handle both 'results' and 'contacts' response formats from Wix API
    // Following the Wix JavaScript SDK documentation
    const results = searchResult.results || searchResult.contacts || [];
    console.log('Processed search results:', results);
    console.log('Results length after processing:', results.length);
